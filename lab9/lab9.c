...
data from? → From standard input (read() from STDIN_FILENO). Whatever you type
is what gets written to the socket.

4. How does the client program end?
   → When read() returns 1 byte or fewer (EOF or empty input). The while-loop
stops and the socket is closed.
*/
