*Questions(from client.c) :
    ...
    : The client connects to 127.0.0.1 on port
      8080. This comes from the sockaddr_in setup where sin_addr is set to
      127.0.0.1 and
        sin_port is set to 8080.

        2. UDP
    or TCP
    ? How do I know
    ? It uses TCP.The socket() call creates a SOCK_STREAM socket,
which means TCP.

    3. Where does the client get the data it sends
    ? It reads from standard input using fgets() inside the main loop
          .Whatever the user types is what gets sent through send()
          .

      4. How does the client exit
    ? The client stops when fgets() returns NULL(EOF) or when the user types an
                                                         empty line.After that,
the loop breaks and the socket gets closed.* /
